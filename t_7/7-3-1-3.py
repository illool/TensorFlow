...
#使用tf.train.QueueRunner创建5个入队操作的线程运行队列的入队操作
...
with tf.Session() as sess:
    ...
    #在你运行任何训练步骤之前，需要调用tf.train.start_queue_runners函数，
    #否则数据流图将一直挂起。tf.train.start_queue_runners 
    #这个函数将会启动输入管道的线程，填充样本到队列中，以便出队操作可以从队列中拿到样本。
    threads = tf.train.start_queue_runners(sess=sess,coord=coord)
    #获取队列中的取值
    for _ in range(30):print(sess.run(out_tensor)[0])
    #循环结束，使用 Coordinator来停止所有线程
    coord.request_stop()
