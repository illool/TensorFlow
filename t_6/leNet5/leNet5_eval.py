...
def evaluate(mnist):
    with tf.Graph().as_default() as g:
            #调整输入数据的格式，输入为一个四维矩阵
        x = tf.placeholder(tf.float32,shape=(leNet5_train.BATCH_SIZE,#第一维表示一个batch里样例个数，BATCH_SIZE=100
                                               leNet5_inference.IMAGE_SIZE,#第二，第三表示图片的尺寸  28*28
                                               leNet5_inference.IMAGE_SIZE,
                                               leNet5_inference.NUM_CHANNELS),name='x_input')#黑白NUM_CHANNELS=1，第四维表示图片的深度，对于RGB 格式的图片，深度为3
        y_ = tf.placeholder(tf.float32,shape=(None,leNet5_inference.OUTPUT_NODE),name='y_input')#OUTPUT_NODE=10，为一个向量
        #定义测试集数据
        xv,yv = mnist.test.next_batch(leNet5_train.BATCH_SIZE)#BATCH_SIZE=100。从测试集中取出BATCH_SIZE=100个测试数据
        reshape_xv = np.reshape(xv,(leNet5_train.BATCH_SIZE,
                                        leNet5_inference.IMAGE_SIZE,
                                        leNet5_inference.IMAGE_SIZE,
                                        leNet5_inference.NUM_CHANNELS))
        test_feed = {x:reshape_xv,y_:yv}
        #调用训练好的模型进行计算前向传播的结果,因为测试时不关注正则化损失的值，因此不用计算正则化损失，train=False表示是训练过程
        y = leNet5_inference.inference(input_tensor=x,reuse=False,train=False,regularizer=None)#开始测试
        #进行验证集上的准确率计算
...
def main(argv=None):
    mnist = input_data.read_data_sets("F:\\PythonPro\\testTensorFlow\\src\\MNIST_data",one_hot=True)
    evaluate(mnist)
