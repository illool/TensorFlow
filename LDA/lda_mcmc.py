...
class CorpusSet(object):
    ...
    def __init__(self):
        ...
        self.words_count = 0                # 数据集中word的数量（排重之前的）,真实word结合
        self.V = 0                          # 数据集中word的数量（排重之后的）,字典,主题中词的先验分布是Dirichlet分布
    ...
    def init_corpus_with_articles(self, article_list):
        ...
        for line in article_list:
            ...
            # 获取word的id
            art_wordid_list = [] #存放word的id,id值为先后读取的顺序
            for word in [w.strip() for w in frags[1:] if w.strip()]:
                #确认是否已经在word库中，在泽直接使用已有的id，要不在就直接
            ...
            if len(art_wordid_list) > 0:
                ...
                self.arts_Z.append(art_wordid_list)#将art_wordid_list放到arts_Z中，art_wordid_list代表的是低m篇文档中的所有词的id
        ...
        # 做相关初始计算--article相关
        self.M = len(self.artids_list)#有多少篇文档
...
class LdaBase(CorpusSet):
    ...
    def __init__(self):
        ...
        self.topics_num = 0         # LDA训练或推断过程中的topic的数量,即self.K值,先验分布是Dirichlet分布
        self.K = 0                  # LDA训练或推断过程中的topic的数量,即self.topics_num值.主题的个数，文档主题的先验分布是Dirichlet分布
        self.twords_num = 0         # LDA训练或推断结束后输出与每个topic相关的word的个数，

        # 基础变量--2
        self.alpha = numpy.zeros(self.K)            # 超参数alpha,K维的float值,默认为50/K  α
        self.beta = numpy.zeros(self.V)             # 超参数beta,V维的float值,默认为0.01   β

        # 基础变量--3
        # 文档中各个词的所属主题,长度为[m][各个文档的词个数]
        self.Z = []                                 # 主题词信息,所有word的topic信息,即Z(m, n),维数为 M * article.size()

        # 统计计数(可由self.Z计算得到)
        self.nd = numpy.zeros((self.M, self.K))     # 文档主题，nd[m, k]用于保存第m篇article中第k个topic产生的词的个数,其维数为 M * K
        self.ndsum = numpy.zeros((self.M, 1))       # ndsum[m, 0]用于保存第m篇article的总词数,维数为 M * 1
        self.nw = numpy.zeros((self.K, self.V))     # 主题词，nw[k, w]用于保存第k个topic产生的词中第w个词的数量,其维数为 K * V
        self.nwsum = numpy.zeros((self.K, 1))       # nwsum[k, 0]用于保存第k个topic产生的词的总数,维数为 K * 1

        # 多项式分布参数变量
        # theta,长度为[m][K] 文章-主题分布
        self.theta = numpy.zeros((self.M, self.K))  # Doc-Topic多项式分布的参数,维数为 M * K,由alpha值影响,服从参数为α的Dirichlet分布
        # phi,长度为[K][n] 词-主题分布
        self.phi = numpy.zeros((self.K, self.V))    # Topic-Word多项式分布的参数,维数为 K * V,由beta值影响,服从参数为β的Dirichlet分布

        # 辅助变量,目的是提高算法执行效率
        self.sum_alpha = 0.0                        # 超参数alpha的和,α为分布的超参数，是一个K维向量
        self.sum_beta = 0.0                         # 超参数beta的和,β为分布的超参数，是一个V维向量
    ...
    def init_statistics_document(self):
        ...
        # 统计计数初始化
        self.nd = numpy.zeros((self.M, self.K), dtype=numpy.int)#M * K
        self.ndsum = numpy.zeros((self.M, 1), dtype=numpy.int)#M * 1
    ...
    def init_statistics_word(self):
        ...
        # 统计计数初始化
        self.nw = numpy.zeros((self.K, self.V), dtype=numpy.int)#K * V
        self.nwsum = numpy.zeros((self.K, 1), dtype=numpy.int)#K * 1
    ...
    def sum_alpha_beta(self):
        ...
        :key: 计算alpha、beta的和
        """
        self.sum_alpha = self.alpha.sum()# alpha求和
        self.sum_beta = self.beta.sum()# beta求和
        return

    def calculate_theta(self):
        """
        :key: 初始化并计算模型的theta值(M*K),用到alpha值，由alpha值影响,服从参数为α的Dirichlet分布
        """
    ...
    def calculate_phi(self):
        """
        :key: 初始化并计算模型的phi值(K*V),用到beta值，由beta值影响,服从参数为β的Dirichlet分布
        """
    ...
    def gibbs_sampling(self, is_calculate_preplexity):
        ...
        for self.current_iter in range(last_iter, last_iter+iters_num):
            ...
            for m in range(self.M):#取第m篇文档的词向量
                for n in range(len(self.Z[m])):#遍历词向量中的所有词
                    w = self.arts_Z[m][n] #单词 w，第m篇文档中的第n个词，单词的主题zm,n，为wm(文档是个词向量)中的第n个词选择主题
                    k = self.Z[m][n]#主题 k，取出第m篇文档中的第n个词的主题，这个Z初始化的时候是随机设定的，在选联过程中是不端迭代的

                    # 统计计数减一
                    self.nd[m, k] -= 1      #文档主题，第m篇文档第k个主题拥有词数目
                    self.ndsum[m, 0] -= 1   #第m篇文档总的主题词数目sum(nd[m])
                    self.nw[k, w] -= 1      #主题词，第k个topic中拥有的数目
                    self.nwsum[k, 0] -= 1   #第k个topic总共产生的w数目
                    ...
                    if self.prior_word and (w in self.prior_word):
                        ...
                        ## 计算theta值，Dir分布的期望，计算文档主题的分布参数theta，全是向量，应用到向量的计算
                    ...
                    # 统计计数加一
                    self.nd[m, k] += 1      #第m篇文档第k个主题的词数目
                    self.ndsum[m, 0] += 1   #第m篇文档总的主题词数目sum(nd[m])
                    self.nw[k, w] += 1      #第k个topic中产生w的数目
                    self.nwsum[k, 0] += 1   #第k个topic总共产生的w数目

                    # 更新Z值，更新主题，更新第m篇文档的第n个值的主题
                    self.Z[m][n] = k
        # 抽样完毕
        return
...
if __name__ == "__main__":
    ...
    if test_type == "train":
        ...
        model.init_train_model("..\data\\", "model", current_iter=0, iters_num=1000, topics_num=2, twords_num=2,data_file="corpus.txt")
        ...
        model = LdaModel()
        model.init_inference_model(LdaModel().init_train_model(r"..\data\\", "model", current_iter=134))
        data = [
    ...
    print(os.getcwd())
    print("词典的数量：",model.V)
    print("主题词的数量：",model.K)
    print("文本主题：",model.Z)
    print("文档：",model.arts_Z)
    # 退出程序
    exit()
