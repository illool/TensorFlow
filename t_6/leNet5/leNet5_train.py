...
def train(mnist):
    #调整输入数据的格式，输入为一个四维矩阵
    x = tf.placeholder(tf.float32,shape=(BATCH_SIZE,#第一维表示一个batch里样例个数
                                         IMAGE_SIZE,#第二，第三表示图片的尺寸
                                         IMAGE_SIZE,
                                         NUM_CHANNELS),name='x_input')#第四维表示图片的深度，对于RGB 格式的图片，深度为3
    ...
    y = inference(input_tensor=x,reuse=False,train=True,regularizer=regularizer)
    ...
    average_y = inference(input_tensor=x,reuse=True,train=False,regularizer=None)
    ...
    #计算在当前batch里所有样例的交叉熵平均值，并加入损失集合，计算误差值
    ...
    learning_rate = tf.train.exponential_decay(LEARNING_RATE_BASE,#基础的学习率，在此基础上进行递减
                                               ...
                                               LEARNING_RATE_DECAY,staircase=True)#学习率衰减速度
    ...
    with tf.Session() as sess:
        ...
        #迭代的训练神经网络
        #+每100轮保存一次模型
        for i in range(TRAINING_STEPS):#TRAINING_STEPS=10000训练10000次
            #将输入数据格式转为一个四维矩阵
            ...
            xs, ys = mnist.train.next_batch(BATCH_SIZE)#BATCH_SIZE=100。从测试集中取出BATCH_SIZE=100个测试数据
